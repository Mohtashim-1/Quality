...
function set_ss_total(frm, cdt, cdn) {
    ...
    frappe.model.set_value(d.doctype, d.name, "ss_qty1", d.ss_major + d.ss_minor + d.ss_critical);
    // frappe.model.set_value(d.doctype, d.name, "ss_qty1", d.ss_critical+d.ss_minor+d.ss_major);
...
frappe.ui.form.on('Daily Checking Inspection CT', {
    ss_qty1: function (frm, cdt, cdn) {
        var d = locals[cdt][cdn];
        var ss_qty1 = 0;
        frm.doc.items.forEach(function (d) {
            ss_qty1 += d.ss_qty1;
        });
        frm.set_value('ss_qty1', ss_qty1);
        // frm.set_value('ss_qty1', 10);
        refresh_field('ss_qty1');
    },

    items_remove: function (frm, cdt, cdn) {
        var d = locals[cdt][cdn];
        var ss_qty1 = 0;
        frm.doc.items.forEach(function (d) { ss_qty1 += d.ss_qty1; });
        frm.set_value('ss_qty1', ss_qty1);
        refresh_field('ss_qty1');
    }
});
...
frappe.ui.form.on('Daily Checking', {
    ...
    total_audit(frm) {
        ...
        frm.set_value('percent', (frm.doc.total_defects/frm.doc.total_audit)*100)
        // frappe.msgprint("1")
...
frappe.ui.form.on("Daily Checking Inspection CT", {
    ...
    short_size(frm, cdt, cdn) {
        ...
        frappe.model.set_value(d.doctype, d.name, "ss_qty1", 0);
    },
